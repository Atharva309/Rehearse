 ...
 * GET/POST /api/student/crm-contact — load and upsert per-contact CRM notes.
 */
...
const CONTACT_KEYS = new Set<CrmContactKey>(["dana_reyes", "dr_kim"]);

type ContactNotesBody = {
  attemptId?: string;
  contactKey?: string;
  role?: string;
  notes?: string;
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Could not load contact notes." }, { status: 500 });
    }
...
/**
 * POST /api/student/crm-contact — upsert contact notes for the owned attempt.
 */
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const body = (await request.json()) as ContactNotesBody;
    const attemptId = body.attemptId?.trim();
    const contactKey = body.contactKey?.trim() as CrmContactKey | undefined;

    if (!attemptId || !contactKey || typeof body.notes !== "string" || typeof body.role !== "string") {
    ...
    if (error || !data) {
      ...
      return NextResponse.json({ error: "Could not save contact notes." }, { status: 500 });
    }
